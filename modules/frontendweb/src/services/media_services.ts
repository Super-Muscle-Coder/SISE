 ...
 * @layer services
 * @description React hooks for media gallery, image upload orchestration, and polling
...
export interface UseMediaUploadActions {
    ...
    ) => Promise<string>; // Returns image_id
...
export function useMediaUpload(): UseMediaUploadActions {
    ...
    const uploadFile = useCallback(
        async (file: File, albumId: number | null, privacyLevel: PrivacyLevel): Promise<string> => {
            ...
            // Validate file
            if (!MEDIA_CONFIG.upload.allowedMimeTypes.includes(file.type as any)) {
                const err = new Error(MEDIA_CONFIG.messages.invalidFileType);
                setState((prev) => ({ ...prev, uploadError: err }));
                throw err;
            }

            if (file.size > MEDIA_CONFIG.upload.maxFileSizeMb * 1024 * 1024) {
                const err = new Error(MEDIA_CONFIG.messages.fileTooLarge);
                setState((prev) => ({ ...prev, uploadError: err }));
                throw err;
            }
            ...
            try {
                ...
                setState((prev) => ({
                    ...
                    indexStatus:
                        confirmResponse.index_status === 'pending'
                            ? IndexStatus.PENDING
                            : (confirmResponse.index_status as IndexStatus),
                    uploadProgress: 100,
                }));
...
export function useImageStatusPolling(imageId: string): UseImageStatusPollingActions {
    ...
    const poll = useCallback(async () => {
        ...
        try {
            ...
            // Ignore abort errors (expected on unmount or re-poll)
            ...
            setState((prev) => {
                ...
                const maxRetriesExceeded = newRetries >= MEDIA_CONFIG.polling.maxPollRetries;
                ...
                return {
                    ...
                    // If max retries exceeded, mark as failed for UI
                    imageMetadata:
                        maxRetriesExceeded && prev.imageMetadata
                            ? {
                                ...prev.imageMetadata,
    ...
    useEffect(() => {
        ...
        pollIntervalRef.current = setInterval(poll, MEDIA_CONFIG.polling.indexStatusPollIntervalMs);
